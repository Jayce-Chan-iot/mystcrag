...
import {
  aiBeadLayoutCandidateToDesignV1,
  designV1ToAgentOutput,
  legacyDesignToAiBeadLayoutCandidate,
...
test("valid AI candidate converts with server-owned enrichment", () => {
  const result = aiBeadLayoutCandidateToDesignV1(validCandidate(), validEnrichment());
...
test("missing AI candidate fields are rejected", () => {
  ...
  assert.equal(aiBeadLayoutCandidateToDesignV1(candidate, validEnrichment()).status, "REJECTED");
});
...
test("unknown or mismatched product IDs are rejected", () => {
  ...
  const unknownResult = aiBeadLayoutCandidateToDesignV1(unknownProduct, validEnrichment());
  ...
  const mismatchResult = aiBeadLayoutCandidateToDesignV1(validCandidate(), mismatchEnrichment);
...
test("AI candidate cannot provide price or cost fields", () => {
  ...
  assert.equal(aiBeadLayoutCandidateToDesignV1(priceAttempt, validEnrichment()).status, "REJECTED");

  const costAttempt = { ...validCandidate(), unitCostMinor: 1 };
  assert.equal(aiBeadLayoutCandidateToDesignV1(costAttempt, validEnrichment()).status, "REJECTED");
});
...
test("restricted effect copy is rejected during compliance normalization", () => {
  ...
  const result = aiBeadLayoutCandidateToDesignV1(candidate, validEnrichment());
...
test("provider non-object values are rejected", () => {
  assert.equal(aiBeadLayoutCandidateToDesignV1("not-json", validEnrichment()).status, "REJECTED");
});
...
test("legacy grouped output has an isolated compatibility adapter", () => {
  const candidate = legacyDesignToAiBeadLayoutCandidate(
    {
      designName: "Legacy",
