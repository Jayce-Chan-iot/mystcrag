...
- The browser holds only a host-only HttpOnly cookie: production `__Host-mystcrag_session`, `Secure`, `SameSite=Lax`, `Path=/`, no `Domain`. Idle expiry is 8 hours and absolute expiry is 7 days. Session identifiers rotate at login, credential renewal, and at least every 15 minutes of accepted activity.
...
- Fastify validates signature, exact issuer, audience, expiry, and no more than 60 seconds clock skew before `(issuer, subject)` mapping. Verification failure is generic `401`; owner mismatch remains generic `403`; provider/JWKS outage fails closed without treating a session as anonymous.
