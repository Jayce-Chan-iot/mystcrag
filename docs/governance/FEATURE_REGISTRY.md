...
| FEAT-009 | Profile/design/order history | `/profile`, design/order list APIs | frontend profile + Design API | FRONTEND | ACTIVE | Owner-scoped through the production BFF/AuthProvider identity boundary |
...
| FEAT-018 | Authentication and user identity | `/auth/**`, same-origin `/api/**` BFF, Auth0 verifier and external-identity mapping | `docs/AUTH_SESSION_CONTRACT.md`; frontend auth boundary; backend auth providers; external identity repository | SOL -> DATABASE/BACKEND/FRONTEND/QA DAG | PARTIAL | Runtime and AUTH-006 54-test gate are integrated; AUTH-007 acceptance is blocked until a numeric session p95 budget is authorized/measured and exact staging/production Auth0 allowlists plus smoke evidence are supplied |
