...
| FEAT-018 | Authentication and user identity | `/auth/**`, same-origin `/api/**` BFF, Auth0 verifier and external-identity mapping | `docs/AUTH_SESSION_CONTRACT.md`; frontend auth boundary; backend auth providers; external identity repository | SOL -> DATABASE/BACKEND/FRONTEND/QA DAG | PARTIAL | AUTH-001 through AUTH-005 and AUTH-008 integrated; AUTH-006 isolated security/full-loop E2E repair and rerun is next; only AUTH-007 may record Feature acceptance |
