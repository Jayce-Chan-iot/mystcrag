...
| FEAT-004 | 2D DIY bracelet editing | `/diy`, `/diy/[id]` | `FlatBraceletEditor` + Bracelet Engine | FRONTEND | ACTIVE | TASK-FE-002 completed and integrated at final candidate `30782261eb63e74f6c22083b9580d4a114e222bc`: direct-manipulation feedback, unobstructed mobile composition with persistent wrist/length/count/price context, explicit sync/conflict/recovery states, and keyboard/44px/reduced-motion accessibility while preserving Bracelet Engine and server authority |
...
| FEAT-026 | Bead asset import assistant | planned `/admin/bead-import` and admin APIs | approved design in `docs/superpowers/specs/2026-08-31-bead-asset-import-assistant-design.md`; no runtime implementation | SOL -> QWEN/GLM DAG | PLANNED | Contract task is the first active dependency; database, worker, backend, frontend, resolver and QA remain gated |
