...
| FEAT-004 | 2D DIY bracelet editing | `/diy`, `/diy/[id]` | `FlatBraceletEditor` + Bracelet Engine | FRONTEND | ACTIVE | TASK-FE-002 Stage A baseline accepted; Stage B targets direct-manipulation feedback, unobstructed mobile composition, persistent wrist/length/count/price context, explicit sync/conflict/recovery states and accessible interaction while preserving Bracelet Engine and server authority |
