...
| Production identity/session contract | `docs/AUTH_SESSION_CONTRACT.md` (`CONTRACT_FROZEN_IMPLEMENTATION_PENDING`) | Next.js BFF, Backend AuthProvider, Database identity mapping, QA/Operations | Auth0 SDK/provider types stay behind adapters; no second public Domain/Design Contract authority |
| Authentication verification | `apps/backend/src/auth/AuthProvider` and authentication pre-handler; provider-neutral semantics frozen by Auth Session Contract | Protected design, recommendation, order and Tarot routes | `SignedTestTokenAuthProvider` is development/test only; Auth0 adapter implementation is pending AUTH-004 |
| User identity persistence | Prisma `User` and actor-scoped foreign keys; future unique `ExternalIdentity(issuer, subject)` mapping | Design, order, publication and Tarot repositories | Provider subject/email are not `User.id`; browser-local profile/address/preferences are presentation state |
