...
| RES-004 | EVIDENCE_SPRAWL | At least 301 tracked files / about 56 MB appear across `artifacts/`, `output/playwright/`, `outputs/`, `qa-captures-*`, `apps/frontend/qa-captures`, and `scripts/ui-qa/{artifacts,qa-captures-final}`, excluding the curated 29 MB `docs/ui-references`. Several ignored patterns remain tracked because they predate `.gitignore`. | P1: repository weight, ambiguous canonical evidence, repeated screenshots | Inventory hashes, preserve cited evidence, archive/dedupe in TASK-REPO-001 |
| RES-005 | MIXED ARTIFACTS | `outputs/` mixes knowledge coverage JSON, spreadsheet deliverables, previews, validation scripts, and inspection NDJSON. Some files are cited by controlling/historical docs, so the directory is not wholly unused. | P1: unsafe bulk cleanup | Classify runtime input vs deliverable vs generated evidence in TASK-REPO-001 |
