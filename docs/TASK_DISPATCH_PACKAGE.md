...
**Package state:** AUTH-001 through AUTH-006 and AUTH-008 are integrated; AUTH-006 final candidate `1412d657236ade40872d71d4468df3d66391040c` is `DONE`; AUTH-007 is `BLOCKED — DEPLOYMENT_ACCEPTANCE_DEFERRED_BY_PRODUCT_OWNER` after its technical replay passed<br>
**Blocking gates:** production acceptance retains the approved same-region staging performance benchmark and byte-exact staging/production Auth0 allowlist/login/logout smoke. AUTH-007 itself must not start unrelated Feature work, redesign authentication or weaken AUTH-006; its deployment-only blocker does not prevent separately registered non-auth Features from proceeding<br>
...
- **ARCHITECTURE CONSTRAINTS:** no weakened contract during merge; conflict resolution preserves the reviewed single authorities; integration order follows DAG; main remains integration-only. Auth0 topology, session/cookie architecture, token custody, Fastify `AuthProvider`, `ExternalIdentity` semantics and AUTH-006 gates remain frozen until deployment acceptance resumes, except under an explicitly registered P0 security-defect task.
...
- **OUT OF SCOPE:** implementing missing worker scope, waiving failed acceptance, implementing any unrelated Feature inside AUTH-007 or deleting branches. Separately registered non-auth Features are governed by their own dependencies and locks and are not blocked by this task.
...
| Task | Owner | Branch | State | Dependency |
| --- | --- | --- | --- | --- |
| TASK-FE-002 Competitive DIY Interaction and Mobile Feedback | **FRONTEND / Qwen** | `task/fe-002-competitive-diy-experience` | `PROPOSED — AWAIT HUMAN PRODUCT OWNER DISPATCH` | TASK-AUDIT-002 accepted; current screenshot baseline captured and explicitly approved before UI changes |
...
TASK-FE-002 reuses existing Design Contract operations and Bracelet Engine layout/fit/slot behavior. Qwen is its sole execution Owner; SOL performs Review and Integration only. Its governance scope is limited to the exact TASK-FE-002 row in `docs/tasks/TASK_REGISTRY.md` and the exact FEAT-004 row in `docs/governance/FEATURE_REGISTRY.md`; no other governance row is writable. Stage A captures the unmodified application under ignored `output/playwright/task-fe-002/before/` and stops for explicit Human Product Owner approval. Only then may Stage B modify the tracked frontend paths and write paired ignored evidence under `output/playwright/task-fe-002/after/`. Browser-capture failure returns `BLOCKED`; it cannot be bypassed with a code-only L3 claim. Screenshots must not be committed, written to the repository root, `docs/ui-references/**` or `apps/frontend/public/**`, and `.gitignore` and other tasks' evidence remain forbidden. TASK-FE-002 may not change backend, any package, root tests, Prisma, package manifests, `pnpm-lock.yaml`, CI/runtime configuration or Auth. TASK-CORE-001 is withdrawn as the first prerequisite; no Core package or workspace importer is authorized. TASK-FE-002 must return one Qwen candidate in `REVIEW` and must not start Mini Program, AI, 3D, Commerce, White Label or another Worker.
