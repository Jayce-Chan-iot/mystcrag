...
- Tarot question text has no database field. The default path stores neither question text nor ciphertext; explicit opt-in may populate only the paired nullable `questionCiphertext` and `questionSavedAt` fields, and public DTO mapping must omit ciphertext. Recommendation writes compare the presence or absence of this pair as part of their immutable operation intent: concurrent opt-in CAS losers reuse the first randomized ciphertext, while a later opt-in cannot mutate a recommendation originally committed without question storage.
