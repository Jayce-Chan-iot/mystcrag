...
### PR #5 CI test-harness repair

| Task | Owner | Branch | Status | Writable paths | Forbidden paths |
| --- | --- | --- | --- | --- | --- |
| TASK-AUTH-CI-001 | SOL (Product Owner explicitly authorized this test-only exception on 2026-09-03) | `task/auth-ci-001-database-connection` | REVIEW | `tests/auth-e2e/helpers/run-state.mts`, `tests/auth-ci-database-url.test.mjs`, `tests/auth-e2e/RUNBOOK.md`, `docs/qa/asset-db-001/CI_REPAIR.md`, exact TASK-AUTH-CI-001 registration in `docs/tasks/TASK_REGISTRY.md` | business runtime, database/schema/migrations, shared contracts, Task 3 branches/files, authentication assertions, sanitizer safety gates, unrelated files; no merge/deploy |

Scope: reproduce and correct worker-side test database connection resolution to match the setup/teardown admin configuration; run regression and full authentication gates before updating PR #5. Preserve the failed evidence sanitizer's fail-closed policy. This is a one-task exception, not a permanent SOL implementation role change.

Local review evidence: [CI repair report](../qa/asset-db-001/CI_REPAIR.md), four regression tests passed after a three-failure red run, full AUTH 54/54 twice with complete teardown, `pnpm validate` passed (root 20/20). Remote CI on the forwarded repair remains a separate gate; no merge is authorized by this record.
