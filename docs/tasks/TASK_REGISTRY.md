...
| TASK-AUTH-003 | DATABASE / GLM | P0 | TASK-AUTH-002 DONE | `task/auth-003-identity-persistence` | REVIEW: GLM delivered `ExternalIdentity` model + migration `20260825100000_add_external_identities`, canonical `ExternalIdentityRepository.findOrProvisionExternalIdentity` in `packages/database/src/repositories/identity.repository.ts`, unit + real-PostgreSQL integration/concurrency tests, `packages/database/src/index.ts` export, and `docs/DATABASE_SCHEMA.md` contract + rollback procedure; candidate `78fe84d`; P0 repair `c75965d` adds the contract-required explicit `onUpdate: Restrict` to the Prisma relation (matching the migration's `ON UPDATE RESTRICT`), asserts both `confdeltype`/`confupdtype` in integration tests, and shows zero `external_identities` drift in `prisma migrate diff`; owned paths unchanged |
