...
| TASK-AUTH-005 | FRONTEND / Qwen | P0 | TASK-AUTH-002, TASK-AUTH-003, TASK-AUTH-004 DONE | `task/auth-005-frontend-session` | IN_PROGRESS: Qwen owns `apps/frontend/app/auth/**`, `apps/frontend/app/api/**`, `apps/frontend/src/features/auth/**`, exact `apps/frontend/proxy.ts`, exact `apps/frontend/next.config.ts` only for the legacy `/api/:path*` rewrite removal/replacement, exact `apps/frontend/src/lib/api/{api-runtime,design-api,design-api.test,tarot-api,tarot-api.test,design-session}.ts*`, exact `apps/frontend/app/layout.tsx`, exact `apps/frontend/components/mobile-bottom-nav.tsx`, co-located tests within those paths, and `docs/INTERACTION_TEST_PLAN.md`; all other paths are locked |
