...
| TASK-AUTH-004 | BACKEND / GLM | P0 | TASK-AUTH-003 DONE | `task/auth-004-backend-provider` | IN_PROGRESS: GLM owns `apps/backend/src/auth/**`, exact `apps/backend/src/app.ts`, exact `apps/backend/src/index.ts`, frozen-contract-required new auth/session routes, co-located backend tests and `docs/API_SPECIFICATION.md`; all other paths are locked |
