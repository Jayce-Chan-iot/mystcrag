...
| TASK-AUTH-006 | QA / GLM | P0 | TASK-AUTH-004, TASK-AUTH-005, TASK-AUTH-008 DONE | `task/auth-006-security-e2e` | IN_PROGRESS: GLM must rebase candidate `3bbf8058d6a236064567ed9f0e9b3bd74597ac42` onto latest `main`; exclusively owns exact `tests/auth-e2e/**` (except any package manifest), exact `.github/workflows/ci.yml`, exact `.gitignore`, and exact `docs/AUTH_006_SECURITY_E2E_REPORT.md`. Repair the independently reviewed harness/CI isolation, sanitized evidence upload, relay allowlist, positive production secure-cookie, cleanup and two-user order-isolation gaps; remove `tests/auth-e2e/package.json`; correct the request helper's double JSON encoding; produce two clean 32/32 runs before REVIEW. No production source is writable |
