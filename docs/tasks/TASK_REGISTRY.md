...
| TASK-AUTH-003 | DATABASE / GLM | P0 | TASK-AUTH-002 DONE | `task/auth-003-identity-persistence` | IN_PROGRESS: GLM owns `packages/database/prisma/schema.prisma`, one new AUTH-003 migration, identity/user repository files and tests under `packages/database/src/**`, `packages/database/src/index.ts`, and `docs/DATABASE_SCHEMA.md`; all other paths remain locked/forbidden |
