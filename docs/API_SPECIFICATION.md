...
Recommendation generation uses active catalog SKUs, authoritative prices, and validated `TAROT_GUIDED` Designs. Materials must also hold enough stock to complete the largest supported wrist on their own (ceil(200 mm / diameter)), so every direction pattern passes downstream inventory validation; materials below that threshold — including zero-stock and low-stock rows — are skipped before ranking, and a catalog that cannot field at least two such materials returns `INVENTORY_CHANGED`. A saved preference port may supply wrist and budget values; the current production adapter has no preference store, so wrist defaults to 155 mm and budget is absent rather than fabricated.
