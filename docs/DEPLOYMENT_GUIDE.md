...
AUTH-005 must construct Auth0 SDK `4.27.0` options explicitly from that single naming set: `appBaseUrl` from app origin; validated `domain` from the exact HTTPS issuer; `clientId`, `clientSecret`, and `secret` from their corresponding values; and `authorizationParameters.audience` from the audience. Callback and logout values are exact validation/allowlist inputs, never inferred from an untrusted Host. Do not depend on implicit `AUTH0_*` or `APP_BASE_URL` aliases.

The stateless SDK Cookie Session configuration is rolling with 28,800-second inactivity and 604,800-second absolute limits. Staging/production use `__Host-mystcrag_session`, Secure, HttpOnly, SameSite Lax, Path `/`, and no Domain; loopback development uses `mystcrag_session` and may disable Secure. `MYSTCRAG_AUTH_SESSION_SECRET` is exactly 32 random bytes encoded as 64 hexadecimal characters and is unique per environment. Supplying `sessionStore` is forbidden; no Redis/session database is provisioned. The SDK's network/session boundary belongs in `apps/frontend/proxy.ts` under AUTH-005 and must run on the frozen security-patched Next.js `16.2.12` baseline; the Proxy bypass and related Next.js High advisories patched in `>=16.2.11` are absent from the resolved tree. Its default `GET /auth/logout` is not the public contract: AUTH-005 must preserve `POST /auth/logout`, validate exact Origin, clear the local cookie first, and then initiate the configured upstream logout.
...
`NEXT_PUBLIC_MYSTCRAG_ACCESS_TOKEN` remains temporary development/test compatibility until AUTH-005. It is forbidden as a production credential path, and browser JavaScript must never receive a production Token.
