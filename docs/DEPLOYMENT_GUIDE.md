...
The stateless SDK Cookie Session configuration is rolling with 28,800-second inactivity and 604,800-second absolute limits. Staging/production use `__Host-mystcrag_session`, Secure, HttpOnly, SameSite Lax, Path `/`, and no Domain; loopback development uses `mystcrag_session` and may disable Secure. `MYSTCRAG_AUTH_SESSION_SECRET` is exactly 32 random bytes encoded as 64 hexadecimal characters and is unique per environment. Supplying `sessionStore` is forbidden; no Redis/session database is provisioned. The SDK's Next.js 16 boundary belongs in `apps/frontend/proxy.ts` under AUTH-005. Its default `GET /auth/logout` is not the public contract: AUTH-005 must preserve `POST /auth/logout`, validate exact Origin, clear the local cookie first, and then initiate the configured upstream logout.
