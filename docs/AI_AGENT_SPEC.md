...
Provider output is untrusted `unknown`. A bounded, compositional risk-category classifier rejects unknown fields, over-length copy, missing or reordered card slots, medical or mental-health Crystal-efficacy claims (including symptom-relief and health-measurement paraphrases), death certainty, deterministic-future language, guaranteed financial or wealth outcomes, and exposed hidden instructions or reasoning. It normalizes and tokenizes English and Chinese copy, then combines conservative predictive/interrogative, time, life/health/finance/relationship/death outcome, material/Crystal, efficacy/action, and hidden-content signals. Ordinary color, styling, outfit, material-placement, and non-predictive reflective uses remain allowed. Provider throws, invalid schemas, and unsafe provider prose select the localized deterministic fallback without discarding the server-owned recommendation designs. Every accepted or fallback result replaces the provider disclaimer with a localized statement that the copy is reflective/design inspiration, not deterministic advice or claimed Crystal efficacy.

Deterministic-future handling covers ordinary predictive questions and answers, not only copy that includes words such as `guaranteed` or `definitely`. Time-bounded job, wealth, application, and relationship outcomes, and diagnostic questions or assertions, stay away from the provider or are discarded on output. Non-predictive reflection and concrete bracelet color, styling, or bead-placement statements remain eligible provider prose.

Question exposure to a future provider is fail-closed. A raw optional question is eligible only when it clearly asks for bracelet/design/color/style/material/outfit/placement guidance or explicitly requests non-predictive self-reflection. An emotional self-description paired with explicit design intent, such as wanting a blue bracelet while feeling anxious, remains a design request and is not treated as a diagnosis. Every ambiguous or general life question, including medical, lifespan, career, visa, relationship, stock, windfall, or other predictive prompts, bypasses the provider and receives deterministic fallback copy. Questions requesting hidden prompts, initialized/internal rules or directives, or chain-of-thought, and questions requiring a certain death or lifespan prediction, are rejected with `COMPLIANCE_BLOCKED`; all are withheld from the provider. Provider metadata and returned results never contain the raw question, prompts, hidden reasoning, or provider failure details.

Provider output is checked independently and conservatively. Definite future assertions about the user's life or external outcomes, diagnoses or medical-condition assertions, Crystal/material efficacy claims, death or lifespan assertions, guaranteed finance/return/windfall claims, and disclosure or use of hidden/private/internal/system instructions, rules, or directives select deterministic fallback. Concrete visual future tense such as a bead that "will sit" beside another bead remains allowed because it describes server-owned design placement rather than a user outcome.
