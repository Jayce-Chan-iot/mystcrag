    ...
    return (
      <div className="flex items-center gap-2 text-sm text-red-600" role="alert" aria-live="assertive">
        ...
        <button
          type="button"
          onClick={() => login()}
          className="underline hover:no-underline focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:ring-offset-2 rounded"
          aria-label="重新登录"
        >
          重新登录
        </button>
  ...
  return (
    <div className="flex items-center gap-3" role="status" aria-live="polite">
      ...
      <button
        type="button"
        onClick={() => logout()}
        className="inline-flex min-h-11 items-center rounded-md px-4 py-2 text-sm font-medium text-[var(--muted)] transition hover:bg-[var(--muted)]/10 focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:ring-offset-2"
        aria-label="退出登录"
      >
        退出
      </button>
